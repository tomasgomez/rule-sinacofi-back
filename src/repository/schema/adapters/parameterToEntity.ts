...
const parameterToEntity = (parameters: IParameterResponse[]): Parameter[] => {
    const parametersAdapted = parameters.map((parameter: IParameterResponse) => {
        ...
        const r: Parameter = {id, name, messageCode, label, type, placeholder, description, priority, rules: rulesAdapted, optionValues: optionValuesAdapted, row, column, defaultValue, createdAt, updatedAt};
        return r;
    })
