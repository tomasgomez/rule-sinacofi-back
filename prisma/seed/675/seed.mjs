...
export const seed675 = async (prisma, rules, options, actions) => {
  
    // seed params
    const params = await seedParams(prisma, params675); 

     // connect schemaToParameter
     await connectSchemaToParameter(prisma, [{ messageCode: schema675.messageCode, parameters: params }]);

     // connect parameterToSchema
     const paramsToSchema = params.map((param) => ({ name: param.name, messageCode: param.messageCode, priority: param.priority, schemas: [schema675] }))
