...
import { params672, schema672 } from "./index.mjs";
...
export const seed672 = async (prisma, rules, options) => {
  
    // seed params
    const params = await seedParams(prisma, params672); 

    // connect schemaToParameter
    await connectSchemaToParameter(prisma, [{ messageCode: schema672.messageCode, parameters: params672 }]);

    // connect parameterToSchema
    const paramsToSchema = params672.map((param) => ({ name: param.name, messageCode: param.messageCode, schemas: [schema672] }))
