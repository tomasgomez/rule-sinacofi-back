...
export const seed679 = async (prisma, rules, options, actions) => {
  
    // seed params
    const params = await seedParams(prisma, params679); 

     // connect schemaToParameter
     await connectSchemaToParameter(prisma, [{ messageCode: schema679.messageCode, parameters: params }]);

     // connect parameterToSchema
     const paramsToSchema = params.map((param) => ({ name: param.name, messageCode: param.messageCode, priority: param.priority, schemas: [schema679] }))
