/*
  ...
  - You are about to drop the column `fieldName` on the `Parameter` table. All the data in the column will be lost.
  ...
  - Added the required column `label` to the `Parameter` table without a default value. This is not possible if the table is not empty.
  - Added the required column `placeholder` to the `Parameter` table without a default value. This is not possible if the table is not empty.
...
*/
-- DropForeignKey
ALTER TABLE "Parameter" DROP CONSTRAINT "Parameter_fieldName_fkey";
...
-- AlterTable
ALTER TABLE "Parameter" DROP COLUMN "fieldName",
...
-- AlterTable
ALTER TABLE "Rules" ALTER COLUMN "messageCode" DROP NOT NULL;
...
-- AddForeignKey
ALTER TABLE "Rules" ADD CONSTRAINT "Rules_messageCode_fkey" FOREIGN KEY ("messageCode") REFERENCES "Schema"("messageCode") ON DELETE SET NULL ON UPDATE CASCADE;
